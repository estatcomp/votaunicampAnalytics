...
```
  ...
  Dado que cada curso pertence a um instituto, podemos vizualizar os votos por curso dentro de cada instituto, e de cada área com a tabela a seguir. Por exemplo, quando filtramos pelo instituto *IMECC*, observamos que contém 4 cursos, todos da área de exatas, e conseguimos observar os votos *"sim", "não"*, e de *"abstenção"* de cada curso.
...
Podemos estimar a proporção de alunos contra a greve através de técnicas estatíticas, como por exemplo, *Boostrap*. Para isto, consideramos que os dados obtidos no site são uma amostra aleatória de todos alunos da Unicamp. Com esta técnica, encontramos que **porcentagem** são contra a greve. 
## fazer o estimador de boodstrap

## explicar  - associado a proporção, temos o ic. não sei se querem colocar. 
...
## não sei escrever também, explicar.

 Se quisermos encontrar o tamanho amostral $n$ necessário na Unicamp para que a probabilidade da nossa estimativa diferir absolutamente do verdadeiro valor por um erro escolhido $B = 0,03$ seja de $\alpha = 0.05$
...
```{r exatas_naoexatas, echo=FALSE, warning=FALSE, message=FALSE, eval=FALSE }
...
```{r prop_exatas_naoexatas, echo=FALSE, warning=FALSE, message=FALSE, eval=FALSE}
...
Uma boa técnica estatística para estimar um parâmetro populacional é o Bootstrap, que considera diversas reamostras da amostra original e calcula a média dessas reamostras para dar uma boa estimativa para o que quer ser estudado.

Nesse caso, queremos estimar a proporção de pessoas que são contra a greve. Para isso, usamos a técnica de Bootstrap, conseguindo os seguintes resultados:
...
res = boot(todosinstitutos, function(dd,i)  sum(as.numeric(dd[i,]$Nao))/sum(as.numeric(dd[i,]$Total)), R = 10000)
res
```
