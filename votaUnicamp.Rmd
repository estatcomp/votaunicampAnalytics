...
Descrever as alterações feitas na tabela(adição de )
...
```{r,echo=FALSE }
proporcaoarea<-matrix(c(sum(exatas$sim)/sum(exatas$total),sum(exatas$nao)/sum(exatas$total),sum(exatas$abstencao)/sum(exatas$total),sum(humanas$sim)/sum(exatas$total),sum(humanas$nao)/sum(humanas$total),sum(humanas$abstencao)/sum(humanas$total),sum(biologicas$sim)/sum(biologicas$total),sum(biologicas$nao)/sum(biologicas$total),sum(biologicas$abstencao)/sum(biologicas$total),sum(artes$sim)/sum(artes$total),sum(artes$nao)/sum(artes$total),sum(artes$abstencao)/sum(artes$total),sum(pespeciais$sim)/sum(pespeciais$total),sum(pespeciais$nao)/sum(pespeciais$total),sum(pespeciais$abstencao)/sum(pespeciais$total)), nrow = 3, ncol=5)

barplot(proporcaoarea,beside = TRUE, names.arg=c("Exatas","Humanas","Biológicas","Artes","P. Especiais"),main = "Comparativo das proporções de 'Sim' por Área", ylab = "Total de Votos", xlab = "Áreas", legend=colnames(exatas[5:7]),col=c("gray","darkblue","lightblue"))
```
...
#### Gráfico das Proporções por Instituto
...
curso200<-filter(graduacao, codigo=="200")
...
NumeroMatriculados = rbind(curso1, curso10, curso101, curso102, curso108,curso11, curso12, curso13, curso2, curso28, curso29, curso34, curso36,curso39, curso4, curso40, curso41, curso42, curso43, curso48, curso49, curso5,curso50,curso51,curso53,curso8,curso89,curso9,curso100,curso107,curso14,curso15,curso27,curso45)

library(DT)
datatable(NumeroMatriculados, options = list(pageLength = 100))
...
##criando novas colunas com proporções
tabelageralm<-mutate(tabelageralm,"Proporção de Sim"=round((tabelageralm$sim/tabelageralm$total),3))
tabelageralm<-mutate(tabelageralm,"Proporção de Não"=round((tabelageralm$nao/tabelageralm$total),3))
...
####Análise do Mestrado por Instituto
```{r}
##tabela e grafico
```
...
```{r, message=FALSE, echo=FALSE}
##ADAPTAR DA GRADUAÇÃO
```


####Grafico do Total de Votos do Doutorado
```{r, echo=FALSE}

barplot(c(sum(doutorado$sim),sum(doutorado$nao),sum(doutorado$abstencao)),legend=colnames(doutorado[5:7]),col=c("gray","darkblue","lightblue"))
```
