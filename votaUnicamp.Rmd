...
##Tamanho da amostra
  ...
  Se quisermos encontrar o tamanho amostral $n$ necessário em um instituto para que a probabilidade da nossa estimativa diferir absolutamente do verdade valor por um erro escolhido $B$ seja de $\alpha = 0.05$, chegariamos em

$$n = \dfrac{N}{4(N-1)D + 1}$$
  
  onde $N$ é a quantidade total de alunos no instituto e $D = \frac{B^2}{z_{\alpha}^2}$, sendo $z_{\alpha} = `r round(qnorm(1-0.05),2)`$.
...
Caso quiséssemos saber uma opinião geral da Unicamp e não separada por instituto, podemos encontrar o tamanho amostral necessário para o mesmo erro e mesmo nível de confiança considerados anteriormente. Isso nos levaria para um tamanho amostral de $$`r round(nTotal,0)`$$
