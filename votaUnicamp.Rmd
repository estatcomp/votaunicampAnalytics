...
Estimamos então que a proporção de alunos contrários à greve corresponde à `r round(res_boot$t0,2)*100`%.  

##Análise de Cluster

Baseando-se em um conhecimento prévio do ambiente acadêmico e, principalmente, comportamental, sabemos que alguns institutos tem opinão semelhante. Acredita-se que o maior envolvimento de alunos que tem como área de estudo Ciências Humanas, por exemplo, sejam mais favoráveis à greve, inclusive pelo enfoque de seus cursos. O que pode ser comprovado ao calcularmos as proporções de votos por área de estudo presentes na Tabela 5.  
...
Portanto, é natural questionar-se se existem alguns institutos com comportamentos semelhantes. Podemos analisar isso graficamente, ao realizarmos um gráfico de componentes. Tal análise gráfica pode ser vista na Figura 2.

```{r plot, echo=FALSE, fig.cap="Figura 2: Gráfico de Componentes para Institutos"}
...
Como podemos ver, o gráfico acima mostra a tendência dos votos por institutos em relação aos vetores que representam as opções de voto. Nota-se que existem alguns institutos aglomerados, o que indica semelhança de proporções e, por isso, cabe um estudo aprofundado do assunto.  
