...
Estes cortes na educação, em específico das universidades, geraram revoltas dos estudantes, o que impulsinou o movimento grevista dos alunos dentro das três maiores universidades do Brasil, USP, UNESP e UNICAMP. 
...
institutos = data.frame(matrix(nrow = 25, ncol = 4))
institutos[1,] = c(sum(imecc$total), sum(imecc$sim), sum(imecc$nao), sum(imecc$abstencao))
institutos[2,] = c(sum(fcf$total), sum(fcf$sim), sum(fcf$nao), sum(fcf$abstencao))
institutos[3,] = c(sum(fcm$total), sum(fcm$sim), sum(fcm$nao), sum(fcm$abstencao))
institutos[4,] = c(sum(fe$total), sum(fe$sim), sum(fe$nao), sum(fe$abstencao))
institutos[5,] = c(sum(fea$total), sum(fea$sim), sum(fea$nao), sum(fea$abstencao))
institutos[6,] = c(sum(feagri$total), sum(feagri$sim), sum(feagri$nao), sum(feagri$abstencao))
institutos[7,] = c(sum(fec$total), sum(fec$sim), sum(fec$nao), sum(fec$abstencao))
institutos[8,] = c(sum(feec$total), sum(feec$sim), sum(feec$nao), sum(feec$abstencao))
institutos[9,] = c(sum(fef$total), sum(fef$sim), sum(fef$nao), sum(fef$abstencao))
institutos[10,] = c(sum(fem$total), sum(fem$sim), sum(fem$nao), sum(fef$abstencao))
institutos[11,] = c(sum(fenf$total), sum(fenf$sim), sum(fenf$nao), sum(fenf$abstencao))
institutos[12,] = c(sum(feq$total), sum(feq$sim), sum(feq$nao), sum(feq$abstencao))
institutos[13,] = c(sum(fop$total), sum(fop$sim), sum(fop$nao), sum(fop$abstencao))
institutos[14,] = c(sum(ft$total), sum(ft$sim), sum(ft$nao), sum(ft$abstencao))
institutos[15,] = c(sum(ia$total), sum(ia$sim), sum(ia$nao), sum(ia$abstencao))
institutos[16,] = c(sum(ib$total), sum(ib$sim), sum(ib$nao), sum(ib$abstencao))
institutos[17,] = c(sum(ic$total), sum(ic$sim), sum(ic$nao), sum(ic$abstencao))
institutos[18,] = c(sum(ie$total), sum(ie$sim), sum(ie$nao), sum(ie$abstencao))
institutos[19,] = c(sum(iel$total), sum(iel$sim), sum(iel$nao), sum(iel$abstencao))
institutos[20,] = c(sum(ifch$total), sum(ifch$sim), sum(ifch$nao), sum(ifch$abstencao))
institutos[21,] = c(sum(profis$total), sum(profis$sim), sum(profis$nao), sum(profis$abstencao))
institutos[22,] = c(sum(iq$total), sum(iq$sim), sum(iq$nao), sum(iq$abstencao))
institutos[23,] = c(sum(ig$total), sum(ig$sim), sum(ig$nao), sum(ig$abstencao))
institutos[24,] = c(sum(ifgw$total), sum(ifgw$sim), sum(ifgw$nao), sum(ifgw$abstencao))
institutos[25,] = c(sum(fca$total), sum(fca$sim), sum(fca$nao), sum(fca$abstencao))

colnames(institutos) = c("Total", "Sim", "Nao", "Abstencao")
rownames(institutos) = c("IMECC", "FCF", "FCM", "FE", "FEA", "FEAGRI", "FEC", "FEEC", "FEF", "FEM", "FENF", "FEQ", "FOP", "FT", "IA", "IB", "IC", "IE", "IEL", "IFCH", "PROFIS", "IQ", "IG", "IFGW", "FCA")
```


# por área

agora colocando os cursos por área 
