...
codigos<-c(tabelageral$codigo)
proporcaosim<-round(c((tabelageral$sim/tabelageral$total)),3)
proporcaonao<-round(c((tabelageral$nao/tabelageral$total)),3)

x<-data.frame(cbind(codigos,proporcaosim,proporcaonao))
colnames(x)<-c("Códigos","Proporção de Sim", "Proporção de Não")
datatable(x,options = list(pageLength=10))

##Exibindo a tabela geral - adição de áreas
library(DT)
datatable(tabelageral, options = list(pageLength = 10))
```
...
```{r sepando por instituto, echo=FALSE}
##colocarm na frente
```
