...
```
  ...
  Isto pode ser comprovado por análises estatísticas, que iremos fazer mais pra frente através de Teste de Hipóteses.
...
Existe um pensamento de que a opinião dos alunos de diferentes áreas são diferentes entre si. Por exemplo, os alunos de Exatas tem uma opinião diferente frente a greve dos alunos de Biológicas e Humanas, e Biológicas e Humanas também diferem entre si. Mas será que esse pensamento realmente acontece? Podemos testar isso usando a teorica estatística de Teste de Hipóteses. 
...
props = c(p_Biologicas,p_Exatas)
names(props) = c("Biológicas", "Exatas")
props
```

E realizando o teste, obtemos o seguinte:
...
props = c(p_Humanas,p_Exatas)
names(props) = c("Humanas", "Exatas")
props
```

E o teste nos leva ao seguinte resultado:
...
props = c(p_Biologicas,p_Humanas)
names(props) = c("Biológicas", "Humanas")
props
```
...
Vamos agora estimar a proporção dos votos através do Bootstrap.
...
Após fazer analises e conhecer melhor os dados, pudemos ver através do teste de hipoteses que possívelmente a proportação para alunos de Exatas e Biológicas são iguais, enquanto que ambas são diferentes da proporção dos cursos de Humanas. Vimos também o comportamento dos votos no momento por instituto
As assembleias dos institutos tem o seu o quórum mínimo de participantes, no IMECC por exemplo, o quórum para ínicio da assembleia se baseia no número de associados ao CAMECC, nos outros intitutos deve ser analogo, mas caso não for temos como base o número de alunos matriculados, vimos nas nossas analises que o ideal seria o número do n sugerido acima para que possamos ter uma boa representatividade na tomada de decisão, sendo para a este projeto ou em assembleias, o ideal é chegarmos nesse número de votantes para estatísticamente termos uma pesquisa efetiva que mostre o perfil de voto dos institutos.
Este semestre foi bem atípico pelo fato desta greve e outros acontecimento, sendo uma ótima iniciativa está pesquisa para que a comunidade da UNICAMP tenha respostas mais plausíveis sobre a representatividade das votações, questionamento este muito feito. Sendo assim vamos continuar inventivando amigos e desconhecido através das redes sociais para continuar votando e termos as respostas que queremos.
