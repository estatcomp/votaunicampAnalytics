...
Definidas essas hipóteses, buscamos, nos dados, evidências a favor de uma delas.Para isso definimos um valor ($\alpha$)=0.05 que é a probabilidade de rejeitarmos a hipótese nula quando ela é verdadeira, ou seja, em 5% das vezes que fizermos esse teste iremos dizer que a proporção de votos "sim" é menos que 50%, quando, de fato, não é.
...
Dado que o p-valor do nosso teste é $1.383e-13$ rejeitamos a hipótese nula e concluimos que a proporção de votos "sim" na graduação não é maior que 50%

###Tamanho amostral por curso de Graduação

O tamanho amostral foi determinado através da fórmula para amostra aleatória simples sem reposição, sendo $p=\frac{1}{2}$ (pior caso):

$$ n=\frac{N}{(4(N-1)D+1)} $$

sendo $D=B²/z²$.

Definimos $B=0.1$, ou seja, o erro será de no máximo 10% e $Z=1.96$, pois aproximamos nossa distribuição para uma normal padrão com 95% de significância, o que nos deu os tamanhos amostrais presentes na tabela abaixo.
...
colnames(tabelatamamostral)=c("Código","Curso","Número de Matriculados","Total de Votos","Tamanho Amostral")

datatable(tabelatamamostral, options = list(pageLength = 100))
```

Observando a tabela acima, podemos ver que em todos os cursos, o número de votos registrados nesta pesquisa não atinge o valor do tamanho amostral calculado,o que deixa claro que os dados são viesados e não representam a população de alunos regularmente matriculados nos curso de graduação da UNICAMP.
...
Na tabela abaixo, filtramos apenas os dados referente aos cursos de *Mestrado* e adicionamos uma coluna com a àrea correspondente de cada curso, e duas colunas com as proporções de votos favoráveis e não favoráveis á greve, respectivamente.
...
Dado que o p-valor do nosso teste é $0.01695$ rejeitamos $H_0$ e concluimos que a proporção de votos "sim" no mestrado não é maior que 50%

##Análises para Doutorado

###Tabela de Dados para Doutorado

Na tabela abaixo, filtramos apenas os dados referente aos cursos de *Doutorado* e adicionamos uma coluna com a àrea correspondente de cada curso, e duas colunas com as proporções de votos favoráveis e não favoráveis á greve, respectivamente.
...
Dado que o p-valor do nosso teste é $0.002789$ rejeitamos $H_0$ e concluimos que a proporção de votos "sim" no doutorado não é maior que 50%
...
colnames(tabelaic2)=c("Código", "Nível", "Total","p","Limite Inferior","Limite Superior")
datatable(tabelaic2, options = list(pageLength = 10))
```

## Conclusão
Primeiramente é preciso esclarecer que os resultados aqui apresentados não são representativos da nossa população, já que os dados, possivelmente são viesados pelo fato da pesquisa não ser anônima, o que pode causar receio nos alunos que são favoráveis a greve. Para registrar o voto é preciso se identificar, fornecendo o seu primeiro nome, curso, RG, e RA o que deixa tais alunos expostos aos docentes que terão acesso a essa pesquisa. Além disso, como foi mostrado acima, nenhum dos cursos que responderam a votação atingiu o número amostral minimo para se ter dados com nível de confiança de 95%.
Dito isto apresentaremos os resultados que obtivemos na análise de dados, reiterando o fato de que estes resultados não representam a população.

Diante de nossas análises e interpretações de gráficos e tabelas, notamos que os alunos de exatas e biológicas, em sua maioria, tendem a serem contrários a greve, enquanto os alunos de humanas e artes são a favor, o que, provavelmente, se deve ao fato desses cursos estarem diretamente ligados às questões sociais.
Até o momento mais da metade dos institutos da UNICAMP estão em greve o que revela um contraste com os resultados da pesquisa, e, além disso evidencia um fato que, por motivos como os relatados anteriormente, tem sido fortemente questionado por docentes e estudantes: a não representatividade das assembléias.

Por fim concluimos que é de extrema importância que cada aluno se posicione a respeito das pautas de greve participando ativamente dos eventos, assembléias e votações online, afim de que se conheça a verdadeira opinião dos estudantes da universidade a respeiro do tema.