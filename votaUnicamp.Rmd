...
Para a análise, apresentaremos as proporções e informações obtidas do banco de dados de forma visual e dinâmica. Os métodos utilizados foram a estimação de proporção por *Bootstrap* e análise de *cluster*.
...
```{r grafbar, echo=FALSE, fig.cap="Gráfico do total de votos por instituto", fig.align="center"}
library(ggplot2)
...
A Tabela 3 completa encontra-se em **Apêndice**
...
A partir dos estudos realizados nesse trabalho, podemos concluir que `r round(res_boot,2)*100`% dos alunos sao contrarios a greve. Contudo, dado a atual situaçao do campus isso nos leva a questionar a representatividade, ora das assembleias estudantis, ora da votaçao dos dados provenientes do site.
Uma possivel justificativa seria do receio de se posicionar em um ambiente "hostil". Muitos alunos contrários à greve podem não ter muito interesse em participar das longas assembleias, ou então se sintam intimidados em declarar seu voto perante uma maioria favorável à greve, portanto os impulsiona a votar online. Em compensação, alunos que votam "sim" podem acreditar que o site provoque evasão às assembleias e, com isso, possivelmente não votariam desta forma.


O código foi produzido de maneira reprodutiva, ou seja, a medida que o número de votos aumente, os resultados serão atualizados simultaneamente.
